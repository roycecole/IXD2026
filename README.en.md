...
| `src/services/tourCore.test.mjs` | 21 | Tour × real input paths (the real `useStore` / `activity` / `createTourRunner`): MIDI knobs and transport keys, voice, phone remote, wheel and Marker all restore first and then act; a hidden tab stops the tour; idle timing; haptics muted during the tour |
...
| `src/lib/captionCss.test.mjs` · `scripts/vite-config.test.mjs` | 3 · 2 | Caption layout rules (above the QR, larger in the audience window) · the build's chunking (the entry never pulls in three) |
...
Total: **557 tests** (`node --test`, including the 37 WebXR tests below).
...
└─ src/
   ...
   ├─ services/                         # resident services (mounted once by Services.jsx): Audience / Gesture / Voice / Haptics / Quality / Tour (tourCore.js = the non-React wiring of the tour, testable in Node)
   ├─ lib/
