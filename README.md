...
| 多人合奏 | 按「多人」秀 QR，手機掃碼變遙控器（PeerJS WebRTC）；多支手機與 MIDI 同時演奏同一片海，全部走同一條 `input()` 路徑（可錄製、可 soft-takeover） |
...
| 鳥群 × 鳥調 | 球外線稿鳥群（V 隊形、拍翅）繞球飛行，群數由**水利署鳥類調查（32720）該海況流域鳥種數**驅動 |
| 物理垃圾 | cannon-es 剛體：瓶 / 袋浮在水面、互相碰撞、被洋流推、被**淨化波推開** |
| AR 實景 | 「實景」以相機當背景（球體浮在真實世界）；**高斯模糊 / 清澈滑桿只作用背景**，球體照常渲染 |
| Marker 快照 | nanoKONTROL2 Track ◀▶＝上/下一場景；Marker Set＝存當下狀態為快照（最多 8 組）、Marker ◀▶ 巡回 |
| 展場模式 | 網址 `?kiosk=1` 直接進演出模式＋閒置吸引；角落統計「合奏 N 人 · 演出 M 次」 |
...
**資料管線**：`public/data/ocean.json` 為建置時快照（水利署水庫水情 + 24h 進流量時序 + 氣象），GitHub Actions 每 6 小時排程 `scripts/fetch-ocean-data.mjs` 刷新並自動 commit。
在 repo **Settings → Secrets and variables → Actions** 加入 `CWA_KEY`（[中央氣象署開放資料平臺](https://opendata.cwa.gov.tw/)申請的授權碼）即改抓 CWA 觀測站（政府源）；未設定時自動退回 Open-Meteo。
