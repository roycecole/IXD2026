...
export default {
  ...
  '隱私：影像只在這台裝置上處理，不會上傳也不會錄下來；關閉開關或離開頁面就會停止相機。手勢模型檔（約 8 MB，來自 Google）與運算程式（WebAssembly，來自 jsDelivr）第一次會下載，之後由瀏覽器快取。': 'Privacy: video is processed on this device only. It is never uploaded or recorded, and the camera stops when you switch this off or leave the page. The hand model file (about 8 MB, from Google) and the WebAssembly runtime (from jsDelivr) are downloaded once, then cached by the browser.',
}
