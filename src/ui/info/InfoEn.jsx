...
const PLAY = [
  ...
  { id: 'tour', body: <><b>Data tour</b>: press <code>T</code>, click "Start tour" on the Data tour card in the panel, or leave the app idle for 30 seconds to step through today's reservoir, the Hualien tide, the moon, Yunlin dust, birds, fish and the river-station constellation (about 100 seconds for a full lap), while large captions below the sphere explain from real data what you are looking at and why it looks that way. Touching the screen, turning a knob or pressing a key (<code>Esc</code> or <code>T</code> also work) stops it and restores the sea you had before; the idle auto tour can be turned off on the card (or forced with <code>?tour=0</code> / <code>?tour=1</code> in the URL).</> },
